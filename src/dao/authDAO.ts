...
class AuthDAO {
  public async getUser(usuario: String) {
    ...
    const result = await pool.then(async (connection) => {
      return await connection.query("select u.idUsuario, u.email, u.password, u.idRol, r.descripcion as rol from usuario u INNER JOIN rol r on u.idRol = r.idRol WHERE email = ?", [
        usuario,
      ]);
    });
    return result;
